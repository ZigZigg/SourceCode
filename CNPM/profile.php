...
<?php
  ...
  $sql = "select * from user where user_id = '$prof_id'";
  $result = mysqli_query($conn,$sql) or die(mysqli_error($conn));
  $row = mysqli_fetch_assoc($result);
  $prof_user_name = $row['user_name'];
  ...
  $sql = "select * from images where user_id = '$prof_id'";
  $result = mysqli_query($conn,$sql) or die(mysqli_error($conn));
  ...
  $sql = "select * from follow where user_id = '$id' and user_following_id = '$prof_id'";
  $result = mysqli_query($conn,$sql) or die(mysqli_error($conn));
...
<title><?php echo $prof_user_name ?>'s profile</title>
...
<!-- !PAGE CONTENT! -->
<div class="" style="overflow: hidden;margin: auto;max-width: 1500px;" >
  ...
  <!-- Username title and bio -->
    ...
    <header class="w3-panel w3-padding-128 w3-center w3-opacity">
      ...
      <?php echo $prof_user_name?> 
      <?php if($follow==0)
            {
             echo'<input type="button" id="followbtn" class="btn btn-default" value="Theo dõi">'; 
            }
            else
              if ($follow==1)
              {
                echo'<input type="button" id="unfollowbtn" class="btn btn-default" value="Đang theo dõi">';
              }
              else
                if ($follow==2) 
                  {
                    echo'<button style"" id="editbtn" class="btn btn-default" style="background-color:whitesmoke;color:blue;">Chỉnh sửa trang cá nhân</button>';
                  } 
      ?></p>
    </header>
  ...
  <!-- Modal for delete or not on click delete -->
  <div class="modal fade" id="myConfirmModal" role="dialog">
    <div class="modal-dialog modal-sm">
      <div class="modal-content">
        ...
        <div class="modal-body">
          <p>Bạn có muốn xóa ảnh này cùng toàn bộ bình luận ?</p>
        </div>
        <div class="modal-footer">
          <button id="metlamroi" type="button" class="btn btn-default" data-dismiss="modal">Hủy</button>
          <button id="confirmdeletebtn" style="background-color: red;" type="button" class="btn btn-default" >Xóa</button>
        </div>
      </div>
    </div>
  </div>
          <div id="buttoncontainer" style="background-color: DimGrey;width: 900px;height: 35px; position: absolute;bottom: 0;">
          <button id="deletebtn" name="deletebtn" style="float: right;" type="button" class="btn btn-default" data-toggle="modal" data-target="#myConfirmModal"><img src="icons/delete.png"></button>
        </div> 
    ...
    <div id="modal01" class="w3-modal" style="background-color: black;">
      ...
      <div id="container" style="overflow:hidden; max-width: 1400px; max-height: 900px; margin:auto;">
        <div id="imgcontainer" class="" style="float:left;position: relative;height: 500px;background-color: white">
          ...
          <div id="imginfo" style="width: 400px;height: 100%; float: left; padding-left: 20px;" >
            <h1 id="caption"></h1>
            <br>
